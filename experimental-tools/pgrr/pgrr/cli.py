...
def capture(port: int, upstream_host: str, upstream_port: int):
    ...
    try:
        ...
        # proxy writes a summary record on shutdown
        try:
...
@click.option(
    "--capture",
    "capture_path",
    default=replay.CAPTURE_FILE,
    show_default=True,
    type=str,
    help="Path to capture JSONL file (queries.json).",
)
@click.option(
    "--host",
    default=replay.TARGET_HOST,
    show_default=True,
    type=str,
...
@click.option(
    "--port",
    default=replay.TARGET_PORT,
    show_default=True,
    type=int,
    help="Target Postgres port.",
)
def replay_cmd(speed: float, capture_path: str, host: str, port: int):
    """Start replay mode (replays captured packets to the target Postgres)."""
    if speed <= 0:
